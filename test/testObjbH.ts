...
function getProperties(date, jsondata) {
  ...
  for (let i = 0; i < 7; i++) {
    const day = Day.init(i, days, timeZoneConvert(date), holidays);
    properties.push({
      Name: day.getDayName(),
...
function generateShiftedTestObjects(initialDate, numberOfDays) {
  ...
  for (let i = 0; i < numberOfDays; i++) {
    const date = shiftDay(new Date(initialDate), i);
    objects.push(
      getTestObject(
...
function generateUnshiftedTestObjects(initialDate, numberOfDays) {
  ...
  for (let i = 0; i < numberOfDays; i++) {
    const date = shiftDay(new Date(initialDate), i);
    const currentDay = timeZoneConvert(new Date(date)).getDay();
    objects.push(
      getTestObjectUS(
        Day.init(
          currentDay,
